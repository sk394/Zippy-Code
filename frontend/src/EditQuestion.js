...
const EditQuestion = () => {
    ...
    const { data, loading, error: fetchError } = useQuery(`questions/${questionId}`);
    ...
    }, [data, loading, questionId]);

    const { data: putData, loading: putLoading, error: putError, putRequest } = useEdit();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await putRequest(`questions/${questionId}`, question);
        } catch (error) {
            console.error(error);
        } finally {
        ...
        <div className="max-w-4xl mx-auto p-6 shadow-md rounded-lg">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end">
                    <button type="submit" className="btn btn-primary" disabled={!isFormValid(question) || putLoading}>
                        Edit Question
                    </button>
                </div>
            </form>
            ...
            {putError && <div className="alert alert-error">{putError}</div>}
        </div>);
};
